...
<div class="slider">
	...
	<div class="native_sliders_groups" <?php if ($page_slider_type == 'layerslider' OR $page_slider_type == 'cuteslider'): ?>style="display: none;"<?php endif; ?>>
		<p><?php _e("Slider group", 'axioma') ?>:</p>
		<?php if (!empty($slides)): ?>
			<div class="sel"><select name="page_slider">
					<option value=""><?php _e("Choose slider group", 'axioma') ?></option>
		...
		<?php else: ?>
			<p><?php _e("You still haven't created any slider group.", 'axioma') ?><br><a href="<?php echo admin_url('post-new.php?post_type=' . TMM_Ext_Sliders::$slug) ?>"><?php _e("Click here", 'axioma') ?></a> <?php _e("to create one.", 'axioma') ?></p>
		<?php endif; ?>
	</div>
	...
	<?php if (function_exists('layerslider_init')): ?>
		<div id="layerslider_slidegroups" <?php if ($page_slider_type != 'layerslider'): ?>style="display: none;"<?php endif; ?>>
			...
			?>
			<p><?php _e("Layerslider's groups", 'axioma') ?>:</p>
			<div class="sel">
				<select name="layerslider_slide_group">
					...
					<?php if (!empty($sliders)) : ?>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<?php _e("You still haven't created any slider group for your Layerslider.", 'axioma') ?>
					<?php endif; ?>
				</select>
			</div>
		</div>
	...
	<?php if (function_exists('cuteslider_init')): ?>
		<div id="cutelider_slidegroups" <?php if ($page_slider_type != 'cuteslider'): ?>style="display: none;"<?php endif; ?>>
			...
			$sliders = $wpdb->get_results("SELECT * FROM $table_name
											...
											ORDER BY id ASC LIMIT 200");
			?>
			<p><?php _e("Cuteslider's groups", 'axioma') ?>:</p>
			<div class="sel">
				<select name="cuteslider_slide_group">
					...
					<?php if (!empty($sliders)) : ?>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<?php _e("You still haven't created any slider group for your Cuteslider.", 'axioma') ?>
					<?php endif; ?>
				</select>
			</div>
		</div>
