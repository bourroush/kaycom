...
<div class="slide-item" id="slide_item_<?php echo $unique_id ?>">
    ...
    <div class="slide-thumb">
		<img src="<?php echo TMM_Helper::resize_image($group['imgurl'], '100*100') ?>" alt="slide" />
		<input type="hidden" name="slides_group[<?php echo $unique_id ?>][imgurl]" value="<?php echo $group['imgurl'] ?>" />
		<a href="#" class="button js_edit_slide" data-slide-id="<?php echo $unique_id ?>"><?php _e('Edit Image', 'axioma'); ?></a>
	</div>

    <a href="#" class="js_delete_slide" slide-id="<?php echo $unique_id ?>" title="<?php _e('Delete Slide', 'axioma'); ?>"><?php _e('Delete Slide', 'axioma'); ?></a>
						...
						<?php foreach ($slider['fields'] as $field_key => $field) : ?>
							...
							<div class="slide-attr">
								...
								<?php if ($field['type'] == 'textinput'): ?>
									...
									<input type="text" class="left" name="slides_group[<?php echo $unique_id ?>][<?php echo $slider_key ?>][<?php echo $field_key ?>]" value="<?php echo @$group[$slider_key][$field_key] ?>" />
								<?php endif; ?>
								...
								<?php if ($field['type'] == 'checkbox'): ?>
									<?php $label_id = uniqid() ?>
									<input id="<?php echo $label_id ?>" type="checkbox" id="" <?php if (@$group[$slider_key][$field_key]): ?>checked<?php endif; ?> class="option_checkbox" name="slides_group[<?php echo $unique_id ?>][<?php echo $slider_key ?>][<?php echo $field_key ?>]" value="<?php echo (int) @$group[$slider_key][$field_key] ?>" />
