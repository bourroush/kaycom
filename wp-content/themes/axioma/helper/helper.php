...
class TMM_Helper {
	...
	public static function get_image($img_src, $alias, $show_cap = true) {
		...
		if (!$new_img_src) {
			if ($show_cap) {
				return TMM_THEME_URI . '/images/no_image/no-image.jpg';
			}
		}
...
//Custom page navigation
	...
	public static function get_monts_names($num) {
		$monthes = array(
			...
			6 => __('July', 'axioma'),
			7 => __('August', 'axioma'),
			8 => __('September', 'axioma'),
			9 => __('October', 'axioma'),
			10 => __('November', 'axioma'),
			11 => __('December', 'axioma'),
		);
	...
	public static function get_days_of_week($num) {
		$days = array(
			...
			1 => __('Monday', 'axioma'),
			2 => __('Tuesday', 'axioma'),
			3 => __('Wednesday', 'axioma'),
			4 => __('Thursday', 'axioma'),
			5 => __('Friday', 'axioma'),
			6 => __('Saturday', 'axioma'),
		);
	...
	public static function get_post_categories() {
		...
		$post_categories[0] = __('All Categories', 'axioma');
				...
				if (is_category() || is_single()) {
					if (is_object($post)) {
						...
						if (!empty($categories)) {
							...
							echo '<a href="' . esc_url(get_category_link($categories->term_id)) . '" title="' . esc_attr(sprintf(__("View all posts in %s", 'axioma'), $categories->name)) . '" >' . $categories->name . '</a></li>';
						}
						//***
						if (is_single()) {
							echo " ";
					...
					if (is_post_type_archive('post')) {
						_e('Blog Archives', 'axioma');
					} elseif (is_post_type_archive(TMM_Portfolio::$slug)) {
						_e('Folio Archives', 'axioma');
					}
	...
	public static function get_the_category_list($separator = '', $parents = '', $post_id = false) {
		...
		if (empty($categories))
			return apply_filters('the_category', __('Uncategorized', 'axioma'), $separator, $parents);
		...
		foreach ($categories as $category) {
			...
			if ($cat == $category->term_id) {
				...
				$thelist .= '<a href="' . esc_url(get_category_link($category->term_id)) . '" title="' . esc_attr(sprintf(__("View all posts in %s", 'axioma'), $category->name)) . '" ' . $rel . '>' . $category->name . '</a></li>';
			}
		}
		...
		if ($page_id > 0) {
			...
			if (!empty($page_settings['headerbg_image'])) {
				...
				switch ($page_settings['headerbg_type_image_option']) {
					case "repeat-x":
						if (!empty($page_settings['headerbg_image'])) {
							$styles['image'] = "style='background: url(" . $page_settings['headerbg_image'] . ") repeat-x 0 0; background-attachment:" . $page_settings['headerbg_type_image_bg_cover'] . "; filter: alpha(opacity = " . ((float) $page_settings['headerbg_opacity']) . "); opacity:" . ((float) $page_settings['headerbg_opacity'] / 100) . "'";
						}
						break;

					case "fixed":
						if (!empty($page_settings['headerbg_image'])) {
							$styles['image'] = "style='background: url(" . $page_settings['headerbg_image'] . ") no-repeat center top fixed; background-attachment:" . $page_settings['headerbg_type_image_bg_cover'] . "; filter: alpha(opacity = " . ((float) $page_settings['headerbg_opacity']) . "); opacity:" . ((float) $page_settings['headerbg_opacity'] / 100) . "'";
						}
						break;

					default:
						if (!empty($page_settings['headerbg_image'])) {
							$styles['image'] = "style='background: url(" . $page_settings['headerbg_image'] . ") repeat 0 0; background-attachment:" . $page_settings['headerbg_type_image_bg_cover'] . "; filter: alpha(opacity = " . ((float) $page_settings['headerbg_opacity']) . "); opacity:" . ((float) $page_settings['headerbg_opacity'] / 100) . "'";
						}
						break;
				}
			}
			...
			if (!empty($page_settings['headerbg_color'])) {
				$styles['color'] = "style='background: " . $page_settings['headerbg_color'] . "; '";
			}
		}
